...
from exodus.core.models.events import AgentChange
...
Always explain what you're doing when using tools.""",
        ...
        config={"max_iterations": settings.get("agent.max_iterations", 10), "verbose": False},
    )
...
class ChatSession:
    ...
    def __init__(
        ...
        self.api_key = (
            api_key
            or settings.get("llm.default_provider_config.api_key")
            or os.getenv("GEMINI_API_KEY")
        )

        if not self.api_key:
            raise ValueError(
                "No API key provided. Set it via --api-key, settings.toml, or GEMINI_API_KEY environment variable"
            )
        ...
        self.model = model or (
            self.agent_definition.llm_config.model
            if self.agent_definition.llm_config
            else settings.get("llm.default_model", "gemini-2.5-flash")
        )
        self.temperature = (
            temperature
            if model
            else (
                self.agent_definition.llm_config.temperature
                if self.agent_definition.llm_config
                else 0.7
            )
        )
        self.max_tokens = max_tokens or (
            self.agent_definition.llm_config.max_tokens
            if self.agent_definition.llm_config
            else settings.get("llm.max_tokens", 4096)
        )
        ...
        while True:
            ...
            if agent_change:
                ...
                try:
                    ...
                    logger.error(
                        f"Handoff failed: Agent '{agent_change.new_agent_name}' not found: {e}"
                    )
