...
class CommandHandler:
    ...
    def agents_command(self, args: Optional[str]) -> bool:
        ...
        try:
            ...
            if not agents_info:
                display.print_system_message("No agents loaded. Make sure agent TOML files exist in exodus/agents/single/")
            else:
                display.print_agents_list(agents_info, current_agent=self.session.agent_definition.name)
        except Exception as e:
